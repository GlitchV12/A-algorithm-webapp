...
import { GridState, Node } from '../types/grid';

const createInitialGrid = (size: number): Node[][] => {
  const grid: Node[][] = [];
...
const calculateHeuristic = (node: Node, goalNode: { x: number; y: number }): number => {
